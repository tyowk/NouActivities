...
function verify(key) {
    ...
    return async (req, res, next) => { try {
        ...
        const processBody = async (rawBody) => {
            if (!await verifyKey(rawBody, signature, timestamp, key)) return res.status(401).json({ code: 538401, message: 'Client Unauthorized' });
        ...
        if (req.body) {
            ...
            req.on('end', async () => { const rawBody = Buffer.concat(chunks); await processBody(rawBody)});
        }} catch (err) { return res.status(500).json({ code: 538500, message: 'Internal Server Error' })};
    };
};

async function registerCommands() { try {
